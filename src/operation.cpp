...
void Operation::LoadDictionary(const std::string& filepath) 
{
    ...
    while (std::getline(file, line)) {
        ...
        // This strips out spaces, punctuation, and hidden Windows carriage returns (\r),
        // while converting everything to lowercase.
        for (char c : line) {
            if (std::isalpha(c)) {
        ...
        // 5. Enforce game rules and insert into Trie
        // We only want words that are 4 letters or longer for the puzzle generator
        if (cleanWord.length() >= 4) {
            
            // NOTE: Change 'Insert' to whatever your actual insertion method is named
            // e.g., _trie.AddWord(cleanWord) or _trie.insert(cleanWord)
